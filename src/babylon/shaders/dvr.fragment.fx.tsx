...
bool isVisible(uint segID) {
  ...
  if (segID > uint(500) && segID < uint(1000)) {
    ...
    if (segID > uint(3000) && segID < uint(3500)) {
    return true;
  }
  return false;
}
...
vec4 calcSmoothColor(vec3 texCoord, float stepsize) {
  ...
  colorCorners[0] = segColorIfVisible(getSegID(vec3(
    texCoord.x, texCoord.y, texCoord.z)));
  colorCorners[1] = segColorIfVisible(getSegID(vec3(
    texCoord.x, texCoord.y, texCoord.z + stepsize)));
  colorCorners[2] = segColorIfVisible(getSegID(vec3(
    texCoord.x + stepsize, texCoord.y, texCoord.z)));
  colorCorners[3] = segColorIfVisible(getSegID(vec3(
    texCoord.x + stepsize, texCoord.y, texCoord.z + stepsize)));
  colorCorners[4] = segColorIfVisible(getSegID(vec3(
    texCoord.x, texCoord.y + stepsize, texCoord.z)));
  colorCorners[5] = segColorIfVisible(getSegID(vec3(
    texCoord.x, texCoord.y + stepsize, texCoord.z + stepsize)));
  colorCorners[6] = segColorIfVisible(getSegID(vec3(
    texCoord.x + stepsize, texCoord.y + stepsize, texCoord.z)));
  colorCorners[7] = segColorIfVisible(getSegID(vec3(
    texCoord.x + stepsize, texCoord.y + stepsize, texCoord.z + stepsize)));

  colorMixed[0] = mixColorsIfVisible(
    colorCorners[0], colorCorners[1], ratio.z);
  colorMixed[1] = mixColorsIfVisible(
    colorCorners[2], colorCorners[3], ratio.z);
  colorMixed[2] = mixColorsIfVisible(
    colorCorners[4], colorCorners[5], ratio.z);
  colorMixed[3] = mixColorsIfVisible(
    colorCorners[6], colorCorners[7], ratio.z);

  colorMixed[4] = mixColorsIfVisible(
    colorMixed[0], colorMixed[1], ratio.x);
  colorMixed[5] = mixColorsIfVisible(
    colorMixed[2], colorMixed[3], ratio.x);
...
vec3 calcGradient(vec3 texCoord, float stepsize) {
  ...
  if (getSegID(texCoord + stepsize * vec3(-1.0,  0.0,  0.0)) != centerSegID)
    { gradient += vec3(-1.0,  0.0,  0.0); }
  if (getSegID(texCoord + stepsize * vec3( 1.0,  0.0,  0.0)) != centerSegID)
    { gradient += vec3( 1.0,  0.0,  0.0); }
  if (getSegID(texCoord + stepsize * vec3( 0.0, -1.0,  0.0)) != centerSegID)
    { gradient += vec3( 0.0, -1.0,  0.0); }
  if (getSegID(texCoord + stepsize * vec3( 0.0,  1.0,  0.0)) != centerSegID)
    { gradient += vec3( 0.0,  1.0,  0.0); }
  if (getSegID(texCoord + stepsize * vec3( 0.0,  0.0, -1.0)) != centerSegID)
...
vec3 phongBlinn(vec3 normal, vec3 eyeDir, vec3 lightDir, vec3 ambientCol,
...
float occlusion(vec3 pos, float stepsize) {
  ...
  if (isVisible(getSegID(pos + stepsize * vec3(-1.0,  0.0,  0.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3( 1.0,  0.0,  0.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3( 0.0, -1.0,  0.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3( 0.0,  1.0,  0.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3( 0.0,  0.0, -1.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3( 0.0,  0.0,  1.0))))
    { occl += 1.0; }

  // 12 edge adjacent voxels
  if (isVisible(getSegID(pos + stepsize * vec3(-1.0, -1.0,  0.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3(-1.0,  1.0,  0.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3( 1.0, -1.0,  0.0))))
  ...
  if (isVisible(getSegID(pos + stepsize * vec3(-1.0,  0.0, -1.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3(-1.0,  0.0,  1.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3( 1.0,  0.0, -1.0))))
  ...
  if (isVisible(getSegID(pos + stepsize * vec3( 0.0, -1.0, -1.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3( 0.0, -1.0,  1.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3( 0.0,  1.0, -1.0))))
  ...
  // 8 corner adjacent voxels
  /*
  if (isVisible(getSegID(pos + stepsize * vec3(-1.0, -1.0,  1.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3(-1.0,  1.0, -1.0))))
    { occl += 1.0; }
  if (isVisible(getSegID(pos + stepsize * vec3(-1.0,  1.0,  1.0))))
...
void main() {
  ...
  while (true) {
    ...
    if (isVisible(segID)) {
      ...
      color.rgb = phongBlinn(
        normal, camDir, lightDir, 0.07 * basecol, 1.0*basecol, vec3(0.5), 16.0);
      //}
